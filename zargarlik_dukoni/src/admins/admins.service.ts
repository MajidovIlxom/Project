...
export class AdminsService {
  ...
  async updatePass(refreshToken: string, updatePass: ChangePassword) {
    ...
    const admin = await this.adminRepo.findOne({ where: { id: decodedToken['sub'] } });
  ...
  async logout(refreshToken: string, res: Response) {
    ...
    const updatedAdmin = await this.adminRepo.update({hashed_refresh_token: null}, {where: {id: adminData.sub}, returning: true})
